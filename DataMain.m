...
clearvars -except data rel_dir_lidar_mat lidar_rel ind new
%close all
clc
...
addpath('/Users/stormmata/Library/Mobile Documents/com~apple~CloudDocs/Courses/Thesis/Conditional Averages/Functions');
...
%% Section 5 - Apply LiDAR offset

    VeerOff = Veer + T.Offset;                                                  % Apply LiDAR offset            [deg]
    
    VeerOff(VeerOff > 360) = VeerOff(VeerOff > 360) - 360;                      % Correct values > 360 degrees  [deg]
    
    VeerOff(VeerOff == 360) = 0;                                                % Set values == 360 to 0        [deg]

%% Section 6 - Find adjusted wind angle

    NacelleMatrix = Nacelle .* ones(size(Veer,1),size(Veer,2));                 % Preallocate array
    
    Diff = rad2deg(angdiff(deg2rad(NacelleMatrix),deg2rad(VeerOff)));           % Calculate smallest angle      [deg]
    
    NormalPart = cosd(Diff);                                                    % Calculate cosine of angle
    
    NormalWind = Shear .* NormalPart;                                           % Cosine projection of velocity [m/s]

%% Section 7 - Compute Area Average of Incident Wind

    AAWind = AreaAveragedPower(T,NormalWind);                                   % Area-averaged wind speed      [m/s]

%% Section 8 - Identify When Area-Average Wind =/= Hub Wind

    AvgComp.Greater = AAWind > Shear(T.HubRow,:);                               % Area average greater than hub height
    AvgComp.Less    = AAWind < Shear(T.HubRow,:);                               % Area average less than hub height

%% Section 9 - Fit Power Law to Shear

    [PL] = PowerLawFit(Shear,T);                                                % Fit power law to shear profiles

%% Section 9 - Fit Power Law to Shear

    [Ekman] = EkmanFit(Shear,T);                                                % Fit power law to shear profiles

%% Section 10 - Conditional Averages and Stats

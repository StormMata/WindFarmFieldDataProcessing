...
%% Section 5 - Find adjusted wind angle
    ...
    [NormalWind,RelBear] = NormalInflow(Nacelle,Veer,Shear);                    % Find cosine projection of freestream wind [m/s]
...
%% Section 7 - Fit Power Law to Shear
    ...
    [PLFull]   = PowerLawFit(Shear,T,'Full');                                   % Fit power law to shear profiles

    [PLInflec] = PowerLawFit(Shear,T,'Inflec');                                 % Fit power law to shear profiles up to inflection point
...
%% Section 9 - Conditional Averages and Stats
    ...
    [WindBins,Mean,Num,Dist,STD,Sig] = CondAvgs(T,Shear,Power,AvgComp);         % Calculate descriptive and inferential stats for data

%% Section 10 - Time Series of Power Error

    [Ep] = ErrorTimeSeries(Shear,Power,WindBins,Mean,Time,T);                   % Calculate and plot error time series for data

%% Section 11 - Direction Shear and Probability Plot

    [PDFs] = ShearCharacterization(Veer,T,PLFull,PLInflec,'PlotOn');            % Calculate and plot PDF for direction and speed shear

%% Section 12 - Alpha/Beta Relationship

    [AlphaBeta] = AlphaBetaRel(Shear,PLFull,PDFs,Power,WindBins,T);              % Calculate and plot the relationship between shear and power
...
dir = rad2deg(angdiff(zeros(1,length(Veer(7,:))),deg2rad(Veer(7,:))));
probplot(dir)
histogram(dir,'Normalization','probability')

function [D,OIndices] = ExtractDataM(C,T,data)
...
for z = 1:length(T.TOI)

    Indices = IntFilter(C, T, z, data);                                                         % Return indices of filtered data
    
    %% Shear Data
    
        Shear = zeros(length(T.Heights),length(data.DateTime));                                 % Preallocate array             [-]
        
        for i = 1:length(T.Heights)                                                             % Retrieve speed shear          [m/s]
    ...
    %% Veer Data
        ...
        for i = 1:length(T.Heights)                                                             % Retrieve direction shear      [deg]
    ...
    %% Power
        ...
        Power = data.(strcat('BHR',num2str(T.TOI(z),'%02.f'))).Power;                           % Retrieve TOI power            [kW]
    ...
    %% Nacelle Heading
        ...
        Nacelle = data.(strcat('BHR',num2str(T.TOI(z),'%02.f'))).NacellePosition_corrected;     % Retrieve nacelle heaing       [deg]
    
        Nacelle = Nacelle(Indices);                                                             % Apply initial filter          [deg]
    
    %% Turbulence Intensity
    
        I = data.(strcat('BHR',num2str(T.TOI(z),'%02.f'))).TurbulenceIntensity;                 % Retrieve turbulence intensity [%]
    ...
    %% Pitch
        ...
        PitchAng = data.(strcat('BHR',num2str(T.TOI(z),'%02.f'))).Pitch;                        % Retrieve blade pitch angle    [deg]
    ...
    % Power
    fprintf('\nPOWER DATA\n\n')
        Equals = sum(Power == data.(strcat('BHR',num2str(T.TOI(z),'%02.f'))).Power(OrigIndices));
    ...
    fprintf('\nNACELLE HEADING DATA\n\n')
        Equals = sum(Nacelle == data.(strcat('BHR',num2str(T.TOI(z),'%02.f'))).NacellePosition_corrected(OrigIndices));
    ...
    fprintf('\nTURBULENCE INTENSITY DATA\n\n')
        Equals = sum(I == data.(strcat('BHR',num2str(T.TOI(z),'%02.f'))).TurbulenceIntensity(OrigIndices));
    ...
    %% Put Data into Structure
        ...
        D.Shear   = [D.Shear Shear];
        D.Veer    = [D.Veer Veer];
        D.Power   = [D.Power Power];
        D.I       = [D.I I];
        D.Time    = [D.Time Time];
        D.Nacelle = [D.Nacelle Nacelle];
        D.Pitch   = [D.Pitch PitchAng];
    
        OIndices.(sprintf('T%2.0f',T.TOI(z))) = OrigIndices;

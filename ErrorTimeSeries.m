...
% Ep.TODmean = mean(reshape(Ep.abs(1:21630),10,[]));
...
%             Ep.TOD(index,:) = Indices .* Ep.abs;
            Ep.TODmean(index) = mean(nonzeros(Indices .* Ep.abs));
...
Ep.TODAxis = duration(minutes(linspace(0,1439,length(Ep.TODmean))),'Format','hh:mm');
